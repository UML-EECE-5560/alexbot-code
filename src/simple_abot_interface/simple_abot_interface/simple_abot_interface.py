...
class SimpleAbotInterface(Node):
    ...
    def run_serial(self):
        ...
        self.get_logger().debug('Received (possibly incomplete) serial message: ' + encoders)
        # if we timed out, \n should NOT be included at the end
        if encoders != None and encoders[-1] != '\n':
            self.encoder_pub.publish(encoders)

        self.ser.write(self.cmd_string)
        self.get_logger().debug('Sent serial command: ' + self.cmd_string)
